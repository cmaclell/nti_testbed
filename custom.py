...
# modified psiturk looks for this attribute to inject as WSGI middleware into
# the flask app
        ...
        if room in games:
            if not games[room].finished[room]:
                ...
                if 'first' in data and games[room].read_instructions[room]:
                    sio.emit("refresh", games[room].role_string(
                        room), room=room)
...
def getChatMessage(sid, data):
    ...
    Sends a message typed into the chat box to the server. It is called when
    the user presses the send button next to the chat box. The text is in
    message, and the id is the id of the user gotten from the psiturk id (the
    same id used in join as shown below)
    """
    uid = connections.get(sid, None)
    game = games.get(uid, None)
    ...
    if data['message'] in new_game_commands.keys():
        if game is not None:
            new_game = new_game_commands[data['message']](
                game.sio, game.teacher, game.student)
...
def onTrainingButtonPress(sid, data):
    ...
    The html interface calls this function when the user clicks any of the
    buttons that the html interface was sent with the function
    getTrainingButtons(). The identifier is the identifier that was sent with
    the button and the id is the id of the user gotten from the psiturk id (the
    same id used in join as shown below)
    """
    uid = connections.get(sid, None)
    game = games.get(uid, None)
...
def register_user(uid):
    ...
    if len(queue) > 1 and uid not in games:
        ...
        if "demo" in a or "demo" in b:
            new_game = pattern.HtmlUnityDemonstrate(
                sio=sio, teacher=a, student=b)
        else:
            new_game = pattern.HtmlUnityApprentice(
                sio=sio, teacher=a, student=b)

        for user in [a, b]:
            games[user] = new_game
            sio.emit("sendTrainingMessage", "SYSTEM: You've been matched as " +
                     games[user].role_string(user), room=user)
            sio.emit("instructions", games[user].role_string(user), room=user)

    else:
        sio.emit("sendTrainingMessage",
                 "SYSTEM: Waiting for a partner.", room=uid)
